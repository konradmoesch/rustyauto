...
fn handle_channel_open_request(message: &Frame, data: &mut AndroidAutoEntityData) {
    ...
    data.system_audio_service_data.write().unwrap().channel_status = ChannelStatus::OpenRequest;
}
...
fn handle_av_channel_setup_request(message: &Frame, data: &mut AndroidAutoEntityData) {
    ...
    data.system_audio_service_data.write().unwrap().setup_status = SetupStatus::Requested;
}
