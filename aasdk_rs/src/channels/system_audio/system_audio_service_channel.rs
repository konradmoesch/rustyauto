...
pub fn run(data: &mut AndroidAutoEntityData, receival_queue_tx: Sender<ReceivalRequest>, messenger: &mut Messenger) {
    if data.system_audio_service_data.read().unwrap().channel_status == ChannelStatus::OpenRequest {
        ...
        data.system_audio_service_data.write().unwrap().channel_status = ChannelStatus::Open;
    }
    if data.system_audio_service_data.read().unwrap().setup_status == SetupStatus::Requested {
        let mut setup_response_message = channels::general_audio::create_av_channel_setup_response(ChannelID::SystemAudio);
        messenger.cryptor.encrypt_message(&mut setup_response_message);
        messenger.send_message_via_usb(setup_response_message);
        receival_queue_tx.send(ReceivalRequest).unwrap();
        data.system_audio_service_data.write().unwrap().setup_status = SetupStatus::Finished;
    }
}
...
pub fn create_channel_open_response_message() -> Frame {
    ...
    //println!("{:x?}", payload);
    let message = messenger::frame::Frame { frame_header, channel_id: ChannelID::SystemAudio, payload };
    message
}
