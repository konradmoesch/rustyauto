...
pub fn run(data: &mut AndroidAutoEntityData, receival_queue_tx: Sender<ReceivalRequest>, messenger: &mut Messenger) {
    if data.media_audio_service_data.read().unwrap().channel_status == ChannelStatus::OpenRequest {
        let mut channel_open_response_message = create_channel_open_response_message();
        messenger.cryptor.encrypt_message(&mut channel_open_response_message);
        messenger.send_message_via_usb(channel_open_response_message);
        receival_queue_tx.send(ReceivalRequest).unwrap();
        data.media_audio_service_data.write().unwrap().channel_status = ChannelStatus::Open;
    }
    if data.media_audio_service_data.read().unwrap().setup_status == SetupStatus::Requested {
        let mut setup_response_message = channels::general_audio::create_av_channel_setup_response(ChannelID::MediaAudio);
